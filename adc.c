...
unsigned short read_adc(unsigned char channel)  //selecting the channel 4(0x04)
{
	unsigned short reg_val;

	/*select the channel*/
	ADCON0 = (ADCON0 & 0xC3) | (channel << 2);   //first clear the ADCON0(  after the set the  ADCON0

	/* Start the conversion */
	GO = 1;
	while (GO);
    
    //first we have to shift the right justify bits to left shift of 8 times after bitwise or to left justify value
